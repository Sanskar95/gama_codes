...
global {
	...
	init {		
		create ClothesAuctionInitiator number: 1
		{
		location <- {20,20,0};
		itemType <- 'Clothes';
		}
		
		create ElectronicsAuctionInitiator number: 1
		{
		location <- {80,80,0};
...
species ClothesAuctionInitiator skills: [fipa] {
	...
	reflex broadcastFirstCommunication when: !sentSecondCommunication and !sentFirstCommunication  {
...
//				add p to: informedParticipants;
				...
				do reject_proposal with: [ message :: p, contents :: ['Wont be able to go ahead with your proposed price, too low'] ];
...
species ElectronicsAuctionInitiator skills: [fipa] {
...
//				add p to: informedParticipants;
				write '\t' + name + ' sending reject_proposal message to ' + p.sender;
				do reject_proposal with: [ message :: p, contents :: ['Wont be able to proposed,already sold'] ];
...
species participant skills: [fipa,moving] {
	...
	int firstProposal;
	bool intrestMetric<- flip(0.5);
	string interestItem <- intrestMetric ? 'Clothes': 'Electronics';
...
/* 	reflex setInterestitem when: !(set_interestitem) {
	...
	reflex arrivedAtAuction when: goingToAuction{
			...
			write self.name + "At " + interestItem +" Auction";
	...
	reflex goBackToInitPoint when: distance_to(self,initPoint)<1 and isNotIdle{
			...
			self.targetPoint <- nil;
			self.interestItem <- self.intrestMetric ? 'Clothes': 'Electronics';
		...
		if(!setBiddingPrice)
		{
		maxPrice <- (int(rnd(0,(firstProposal)/2)));
...
experiment runMultipleAuction type: gui {
	output {
		display my_display type:opengl {
			species ClothesAuctionInitiator aspect:base;
			species ElectronicsAuctionInitiator aspect:base;
