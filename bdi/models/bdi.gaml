...
global {
	...
	int sumHappy <- 0 update: sum(((MetalHead) collect (each.happy)) + (ChillPerson) collect (each.happy)+ (Gamer) collect (each.happy));
	int sumDrunk <- 0 update: sum(((MetalHead) collect (each.drunk)) + (ChillPerson) collect (each.drunk)+ (Gamer) collect (each.happy));
	
	int ChillGuyWantsToDance <- 0 update: ChillPerson count (each.wantsToDance>wantsToDanceLimit);
	int ChillGuyNotWantToDance <- 0 update: ChillPerson count (each.wantsToDance<=wantsToDanceLimit);
	...
	init {		
		...
		create MetalHead number: 4;
		create ChillPerson number: 4;
...
species Gamer skills: [fipa,moving] control:simple_bdi {
	...
	predicate wants_to_do_something <- new_predicate("wants to do something at any cot") ;
	predicate wants_to_go_somewhere <- new_predicate("wants to go somewhere at any cost ") ;
	predicate has_decided_where_to_go <- new_predicate("has decided where to go at any cost") ;
	predicate stop_and_have_a_conversation <- new_predicate("stop and have a conversation cost instead") ;
...
species ChillPerson skills: [fipa,moving] control:simple_bdi {
    ...
    init {
        ...
        do add_desire(wants_to_do_something); 
            ...
            perceive when: (self.delayOK and self.delayOK) target: ChillPerson where (each.talking=false) in: talk_range {
        	if(!talking and delayOK)
        	{
	...
	plan initConversation intention: stop_and_have_a_conversation instantaneous: true {
		    ...
		    list<string> possible_people_to_talk_to <- get_beliefs_with_name(who_to_talk_to) collect (string(get_predicate(mental_state (each)).values["name_value"]));
			ChillPerson talkTo <- nil;
			list<ChillPerson> talkToList <- ChillPerson where (each.name=possible_people_to_talk_to[0]);	
			write name+"the list: "+talkToList;
			...
			ask talkTo {
				...
				if(myself.isBad)
				{
						...
						if(self.startafight>50)
						{
							...
							if(myself.winOrLoose>self.winOrLoose)
							{
								write myself.name+" wins the fight";
								myself.winOrLoose<-myself.winOrLoose+5;
								myself.happy<-myself.happy+5;
								self.happy<-self.happy-5;
								...
								self.winOrLoose<-myself.winOrLoose+5;
								self.happy<-self.happy+5;
								myself.happy<-myself.happy-5;
							}
...
species MetalHead skills: [fipa,moving] control:simple_bdi {
